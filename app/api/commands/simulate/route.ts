  ...
  return withMiddleware(async (req: NextRequest) => {
    try {
      ...
      if (!validationResult.success) {
        throw new ValidationError('Invalid simulation request', validationResult.error.errors);
      }
      ...
      if (request.sessionId) {
        sessionManager.addCommandToHistory(request.sessionId, `Simulate: ${request.command}`);
      }
    ...
    try {
      ...
      if (command) {
        ...
        return createSuccessResponse({
          ...
          suggestions: parsed.isValid ? [] : CommandParser.suggestCorrections(command),
          parsedCommand: parsed.isValid ? {
            command: parsed.command,
            flags: parsed.flags,
            args: parsed.args,
          } : null,
          extractedTechStack: parsed.isValid ? CommandParser.extractTechStack(parsed) : [],
        });
      } else {
        // Return general help
